...
if(isset($_POST['app']))
{  $select="select icon,name,dink from
          ((select icon,name,dink from ".$uname."_gtable)
           union all
	      (select icon,name,dink from ".$uname."_btable)
	       union all
	      (select icon,name,dink from ".$uname."_qtable))as a where a.name='".$_POST['app']."'";	
   ...
   if($res){
	...
	echo " <table style='line-height:5px;'> ";
	  foreach($arr as $key => $value){
		if($value[0]!=null){
		 echo "<tr>"; 
	     echo '<td width="220" align="left"><img src="pws_icon.php?name='.$value[1].'" width="50" height="50"></td>';
         echo '<td width="120" align="left">'.$value[1].' </td>';
         echo  '<td width="270" align="right"> ';
                 ?>
				 <button onclick=" top.location='<?php echo $value[2];?>';">官网下载</button></a>
				 <?php
            echo '<a href=""><input type="button" value="查看" /></a>
...
// 查询数据库记录总数
$str1="select count(*) as total from
          ((select icon,name from ".$uname."_gtable)
           union all
	      (select icon,name from ".$uname."_btable)
	       union all
	      (select icon,name from ".$uname."_qtable))as a ";
 ...
 $str = "select icon,name,dink from
      ((select icon,name,dink from ".$uname."_gtable)
       union all
	  (select icon,name,dink from ".$uname."_btable)
	   union all
	  (select icon,name,dink from ".$uname."_qtable)) as a ";
    if($page!="")
       {
...
<body>
   <div align="center">
    <table style="line-height:5px;"> 
       ...
       <div>
         <?php if($count>0){?>
         <a href="pws_select_allc.php?page=1"><input type="button" value="首页"/></a>
        <?php if($count>2){ if($page!=1){?>
         <a href="pws_select_allc.php?page=<?php echo $page-1;?>)" ><input type="button" value="上一页"/></a>
        <?php } if($page!=$endpage){?>
         <a href="pws_select_allc.php?page=<?php echo $page+1;?>" ><input type="button" value="下一页"/></a>
        <?php } ?>
         <a href="pws_select_allc.php?page=<?php echo $endpage;?>"><input type="button" value="尾页"/></a>
        <?php }}?>
