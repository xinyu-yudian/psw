...
// 查询数据库记录总数
$str1="select count(*)  from (select icon,name,dink from ".$uname."_qtable) as q";
 $result1 = mysqli_query($conn,$str1);     
  ...
  // 每次查询从第$lim条开始，查询$pageSize条
   $str = "select icon,name,dink from (select icon,name,dink from ".$uname."_qtable) as q";
    if($page!="")
       {
...
<body>
   <div align="center">
    <table style="line-height:5px;"> 
       ...
       <div>
       <?php if($count>0){?>
         <a href="pws_select_qitas.php?page=1"><input type="button" value="首页"/></a>
        <?php if($count>2){ if($page!=1){?>
         <a href="pws_select_qitas.php?page=<?php echo $page-1;?>)" ><input type="button" value="上一页"/></a>
        <?php } if($page!=$endpage){?>
         <a href="pws_select_qitas.php?page=<?php echo $page+1;?>" ><input type="button" value="下一页"/></a>
        <?php } ?>
         <a href="pws_select_qitas.php?page=<?php echo $endpage;?>"><input type="button" value="尾页"/></a>
        <?php }}?>
...
<script language="javascript">
   function del(name){ 
    ...
    if (false === confirm('是否真的要删除?') )return; 

    location.href = 'pws_delect.php?appname=' + name; 
} 
</script>
